...
// Funzione alternativa per trovare il primo bit impostato
static inline int find_first_set_bit(uint32_t value) {
    for (int i = 0; i < 32; i++) {
        if (value & (1 << i)) {
            return i + 1; // Restituisce la posizione del primo bit impostato
        }
    }
    return 0; // Nessun bit impostato
}
                ...
                switch(state) {
                    ...
                    case STATE_ALARM:
                        ...
                        if(sys.alarm == Alarm_HardLimit) {
                            ...
                            if(bit_count(hard_limits.mask) == 1 && (axis = find_first_set_bit(hard_limits.mask)) != -1)
